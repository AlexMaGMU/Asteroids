...
//    for (Point p : shape) {
  ...
  */
  ...
  /*
   *ARGUMENTS:       other, another Polygon object
   *RETURN:          collided, a boolean
   *DESCRIPTION:     returns a boolean representing whether
   *				 or not one polygon has overlapped with
   *				 another polygon
   *ASSUMPTIONS:     Assumes existence of Polygon other
   */
  public boolean collides(Polygon other) {
	  boolean collided = false;
	  for (int c = 0; c < this.getPoints().length; c++) {
		  collided = collided || (other.contains(this.getPoints()[c]));
	  } // end of for loop: checking all the points of this Polygon
	  return collided;
  } // end of method collides
  
  
  /*
   *ARGUMENTS:       brush, a Graphics object
   *RETURN:          void
   *DESCRIPTION:     draws the current Polygon object
   *				 using brush
   *ASSUMPTIONS:     Assumes the current Polygon object
   *				 is legitimate
   */
  public void paint(Graphics brush) {
		...
		} // end of for loop: going through all the Points in the Point[] array
		brush.drawPolygon(x, y, points.length);

	} // end of paint method for Polygon
}